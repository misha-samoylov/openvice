...
public:
	...
	TxdStore& Txd() { return m_txdStore; }

	const TxdStore& Txd() const { return m_txdStore; }



	/* Legacy flat list kept in sync for callers that still scan globally. */
	...
	/* Case-insensitive. Prefer current TXD, then parent, then anywhere. */
...

	{
		...
		GameMaterial* cur = const_cast<TxdStore&>(m_txdStore).FindTexture(name);
		...
		if (cur) {
			...
			for (int i = 0; i < (int)m_textures.size(); i++) {
				...
				if (_stricmp(m_textures[i].name, name) == 0 &&
					...
					m_textures[i].width == cur->width &&

					m_textures[i].height == cur->height &&

					m_textures[i].source.size() == cur->source.size())
